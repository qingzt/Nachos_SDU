...
Machine::Run()
{
    Instruction *instr = new Instruction;  // storage for decoded instruction 加载指令

    if(DebugIsEnabled('m'))
        printf("Starting thread \"%s\" at time %d\n",
	       currentThread->getName(), stats->totalTicks);
    interrupt->setStatus(UserMode);//切换为用户模式
    for (;;) {
        OneInstruction(instr);//不断运行指令
	interrupt->OneTick();//运行一条指令，步进一次时钟
...
Machine::OneInstruction(Instruction *instr)
{
    ...
    // Fetch instruction 
    if (!machine->ReadMem(registers[PCReg], 4, &raw))//从内存中读出一条指令
	...
	break;
      ...
      case OP_SYSCALL://当前指令是系统调用，调用异常处理程序
