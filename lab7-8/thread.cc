...
Thread::Finish ()
{
    ...
    List *ReadyList=scheduler->getReadyList();//获取就绪队列
...
int Thread::Join(int SpaceID){
    ...
    //判断等待的进程是否在已终止的队列中
    Thread *thread;
    List *terminatedList=scheduler->getTerminatedList();//获取已终止的队列
    List *waitingList=scheduler->getWaitingList();//获取等待的队列
